...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("stock_model_training.log"), logging.StreamHandler()],
)
...
def create_features(data_window, prediction_days=5):
    ...
    try:
        ...
        ).astype(int) - ((sma5 < sma20) & (sma5.shift(1) >= sma20.shift(1))).astype(int)
        features["sma20_cross_sma50"] = (
            (sma20 > sma50) & (sma20.shift(1) <= sma50.shift(1))
        ).astype(int) - ((sma20 < sma50) & (sma20.shift(1) >= sma50.shift(1))).astype(
            int
        )
    ...
    try:
        ...
        # 安全计算比率，避免除以0和无穷大值
        features["bb_upper_ratio"] = pd.Series(
            np.where(upperband > 0, close / upperband, 1)
        )
        features["bb_lower_ratio"] = pd.Series(
            np.where(lowerband > 0, close / lowerband, 1)
        )
        features["bb_width"] = pd.Series(
            np.where(middleband > 0, (upperband - lowerband) / middleband, 0)
        )
    ...
    for i in range(1, prediction_days + 1):
        future_min_prices = pd.concat(
            [future_min_prices, pd.Series(data_window.Close).shift(-i)], axis=1
        ).min(axis=1)

    # 计算股票的历史波动率
    hist_volatility = features["returns_1d"].rolling(20).std().mean() * np.sqrt(
        252
    )  # 年化波动率
    ...
    features["target"] = (
        (expected_return > up_threshold) & (max_drawdown < 2 * up_threshold)
    ).astype(int)
...
def train_master_model(
    ...
    for ticker_idx, ticker in enumerate(ticker_list):
        ...
        for retry in range(max_retries):
            try:
                ...
                logging.info(
                    f"下载 {ticker} 的历史数据 (从 {start_date} 到 {today})..."
                )
                data = data_service.get_stock_data(ticker, start_date, today)
                ...
                try:
                    ...
                    if "^" not in ticker:  # 非指数
                        ...
                        market_data = data_service.get_market_index_data(
                            start_date,
                            today,
                        )

                        if (
                            hasattr(market_data.Close, "values")
                            and market_data.Close.values.ndim > 1
                        ):
                        ...
                        features["market_returns"] = (
                            features["market_returns"].ffill().fillna(0)
                        )
                        ...
                        market_trend = (market_ma50 > market_ma200).astype(
                            int
                        )  # 1=牛市, 0=熊市
                        features["market_trend"] = (
                            market_trend.reindex(features.index).ffill().fillna(0)
                        )

                        # 添加VIX指数作为市场恐慌指标
                        try:
                            vix_data = data_service.get_vix_data(
                                start_date,
                                today,
                            )
                            # 确保VIX数据正确对齐
                            ...
                            # 先将VIX数据重索引到特征索引
                            vix_close_aligned = vix_close.reindex(
                                features.index
                            ).ffill()
                            ...
                            # 使用对齐后的数据进行比较
                            features["vix_trend"] = (
                                vix_close_aligned > vix_ma20_aligned
                            ).astype(int)
                        except Exception as e:
                            ...
                            if "align" in str(e):
                                logging.debug(
                                    "索引对齐错误。VIX索引示例: %s, 特征索引示例: %s",
                                    (
                                        str(vix_close.index[:3])
                                        if "vix_close" in locals()
                                        else "未知"
                                    ),
                ...
                logging.error(
                    f"处理股票 {ticker} 时出错 (尝试 {retry+1}/{max_retries}): {str(e)}"
                )
    ...
    if len(all_features) < 1000:
        logging.warning(
            f"警告: 总数据量不足 ({len(all_features)} 行)，模型可能表现不佳"
        )
    else:
    ...
    extra_cols = [
        col for col in all_features.columns if col not in ["target"] and "temp" in col
    ]
    if extra_cols:
    ...
    for col in X.columns:
        ...
        if upper_mask.sum() > 0 or lower_mask.sum() > 0:
            print(
                f"特征 {col} 中有 {upper_mask.sum()} 个上限极端值和 {lower_mask.sum()} 个下限极端值"
            )
            # 用上下界值替换极端值
    ...
    if high_corr_cols:
        ...
        print(
            f"示例: {high_corr_cols[:5] if len(high_corr_cols) > 5 else high_corr_cols}"
        )
    ...
    importance_selector = SelectFromModel(
        RandomForestClassifier(n_estimators=100, random_state=42), threshold="mean"
    )
    ...
    feature_importances = list(
        zip(X_train.columns, importance_selector.estimator_.feature_importances_)
    )
    feature_importances.sort(key=lambda x: x[1], reverse=True)
    for i, (feature, importance) in enumerate(feature_importances[:10]):
        print(
            f"  {i+1}. {feature}: {importance:.4f} {'(已选择)' if feature in selected_features else ''}"
        )

    print("\n重要特征 (基于RFE):")
    for i, feature in enumerate(rfe_features[:10]):
        print(
            f"  {i+1}. {feature} {'(已选择)' if feature in selected_features else ''}"
        )
    ...
    try:
        if isinstance(X_train.index, pd.DatetimeIndex):
            ...
            days_diff = pd.Series(
                [(max_date - d).days for d in X_train.index], index=X_train.index
            )
            ...
            logging.info(
                f"使用时间衰减权重: 最大权重={sample_weights.max():.2f}, 最小权重={sample_weights.min():.2f}"
            )
            ...
            if isinstance(max_date, (pd.Timestamp, dt.datetime)):
                ...
                sample_weights = sample_weights / sample_weights.mean()
                logging.info(
                    f"使用MultiIndex时间衰减权重: 最大权重={sample_weights.max():.2f}, 最小权重={sample_weights.min():.2f}"
                )
            else:
                logging.warning(
                    f"索引的日期部分不是日期类型 ({type(max_date)}), 不使用时间衰减权重"
                )
                sample_weights = None
        else:
            logging.info(
                f"索引不是日期类型 ({type(X_train.index)}), 不使用时间衰减权重"
            )
            sample_weights = None
    except Exception as e:
    ...
    for name, model in models.items():
        try:
            ...
            if (
                ...
                and isinstance(X_train.index, pd.DatetimeIndex)
                and isinstance(sample_weights, (pd.Series, np.ndarray))
            ):
                try:
                    model.fit(
            ...
            # 计算上涨和下跌的预测准确率
            up_accuracy = (
                (y_pred[y_val == 1] == 1).mean() if (y_val == 1).sum() > 0 else 0
            )
            down_accuracy = (
                (y_pred[y_val == 0] == 0).mean() if (y_val == 0).sum() > 0 else 0
            )
            ...
            print(
                f"精确率: {precision:.4f}, 召回率: {recall:.4f}, F1分数: {f1:.4f}, AUC: {auc:.4f}"
            )
            print(
                f"上涨预测准确率: {up_accuracy:.4f}, 下跌预测准确率: {down_accuracy:.4f}"
            )
    ...
    for res in model_results:
        print(
            f"{res['name']}: F1={res['f1']:.4f}, Accuracy={res['accuracy']:.4f}, AUC={res['auc']:.4f}"
        )
